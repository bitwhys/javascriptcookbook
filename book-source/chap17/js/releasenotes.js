...
;(function($) {
    ...
    var releases = {
      ...
      loadEvents: function() {
        ...
        if (settings.showDescription)
          this.$el.delegate(".issue", "click", function() {
            _this.loadIssueDesc(this)
            return false
          })
        if (settings.showComments)
          this.$el.delegate(".btnComments", "click", function() {
      ...
      loadIssueDesc: function(el) {
        ...
        if (!$bigIssue.is(":visible")) {
          ...
          $bigIssue
            .find(".container")
            .html(components.bigIssue(data, settings.showComments))
          $bigIssue.slideDown()
        } else {
          $bigIssue.slideUp()
        }
      ...
      showMilestones: function(milestones) {
        ...
        $.each(milestones, function(i, milestone) {
          ...
          milestone.prettyDate = milestone.due_on
            ? _this.getPrettyDate(milestone.due_on)
            : _this.getPrettyDate(milestone.created_at)
      ...
      showIssues: function(issues) {
        var _this = this
        var jqMilestone = _this.$el.find(
          "[data-id-milestone=" + issues[0].milestone.number + "]"
        )
        ...
        if (settings.showDescription)
          jqMilestone.find(".issue").addClass("cursor")
      },
      ...
      sortDate: function(milestone1, milestone2) {
        milestone1.dateTest = milestone1.due_on
          ? milestone1.due_on
          : milestone1.created_at
        milestone2.dateTest = milestone2.due_on
          ? milestone2.due_on
          : milestone2.created_at
  ...
  var components = {
    ...
    bigIssue: function(data, showComment) {
      var commentLink =
        data.comments != 0 && showComment
          ? '<a data-issue-id="' +
            data.number +
            '" href="#" class="btnComments btn">See Comments</a>'
          : ""
